...
CLoginDialog::CLoginDialog( wchar_t *lpszName )
   : CDialog( lpszName )
{
...
afx_msg void CLoginDialog::OnLogin() 
{
   ...
   // validate password
   int j = wcscmp( m_szPassword, L"PassWord" );

   if ( *m_szUserid != '\0' && j == 0 ) 
   {
      MessageBox( m_szUserid, L"Access Granted", 
                  MB_ICONINFORMATION );
      EndDialog( IDOK );        // Report login success
   }
   else 
   {
      MessageBox( L"Invalid userid or password", 
                  L"Login error", 
                  MB_ICONEXCLAMATION );

      pPassword->SetWindowText( L"" );  // clear password
...
//CMainWin::CMainWin()
//{
//   Create( NULL, L"Application Window",
//           WS_OVERLAPPEDWINDOW,
//           rectDefault );
//}
...
//void CMainWin::login()
//{
//   CLoginDialog loginDialog( L"Login" );
//
